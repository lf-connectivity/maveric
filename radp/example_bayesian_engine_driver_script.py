...
"""
This script runs training and inference, in memory, using the BayesianDigitalTwin
(an RADP RF Digital Twin model). Here, we do not instrument the dockerized service.
Instead, we simply demonstrate the library methods in action.
"""
...
def augment_ue_data(ue_data_df: pd.DataFrame, site_configs_df: pd.DataFrame):
    """
    This method takes in UE data and site config and augments UE data by
    creating new training features in (e.g. log distance and relative bearing)
    """
...
def split_training_and_test_data(ue_data_df: pd.DataFrame, test_size: float):
    """
    Split the UE data into training and test data according to the defined test size.
    Return the training and test data mapped to cell_id
    """
...
if __name__ == "__main__":
    ...
    for cell_id, training_data in cell_id_training_data_map.items():
        bayesian_digital_twin_map[cell_id] = BayesianDigitalTwin(
            ...
            # x_columns=[CELL_LAT, CELL_LON, CELL_EL_DEG, LOG_DISTANCE, RELATIVE_BEARING],
