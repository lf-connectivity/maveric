...
try:
    ...
    import sys
    import os
    # Assumes MAVERIC_ROOT is four levels up from .../utils/tests/
...
class TestTrafficDemandModel(unittest.TestCase):
    ...
    def test_assign_space_types_invalid_params(self):
        ...
        # Mismatched lengths
        self.assertEqual(self.traffic_model._assign_space_types_to_polygons(polygons, {"types": ["A"], "proportions": [0.5, 0.5]}), [])
        # Empty types/proportions
        self.assertEqual(self.traffic_model._assign_space_types_to_polygons(polygons, {"types": [], "proportions": []}), [])
