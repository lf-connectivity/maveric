...
class RADPFileSystemHelper:
    ...
    def load_simulation_metadata(simulation_id: str) -> Dict:
        ...
        metadata_file_path = RADPFileSystemHelper.gen_simulation_metadata_file_path(simulation_id)
        try:
            with open(metadata_file_path, "r") as json_file:
                return json.load(json_file)
        except Exception as e:
            logger.exception(f"Exception occurred while loading metadata for simulation: {simulation_id}: {e}")
            raise e
    ...
    def save_simulation_metadata(sim_metadata: Dict, simulation_id: str):
        ...
        try:
            ...
            logger.exception(f"Exception occurred while saving metadata for simulation: {simulation_id}: {e}")
            raise e
    ...
    def save_simulation_ue_data(simulation_id: str, ue_data_file_path: str):
        ...
        """
        sim_ue_data_file_path = RADPFileSystemHelper.gen_simulation_ue_data_file_path(simulation_id)
        try:
    ...
    def save_simulation_cell_config(simulation_id: str, config_file_path: str):
        ...
        """
        sim_config_file_path = RADPFileSystemHelper.gen_simulation_cell_config_file_path(simulation_id)
    ...
    @staticmethod
    def gen_stage_output_file_path(stage: SimulationStage, hash_val: str, batch: int) -> str:
    ...
    def gen_sim_output_zip_file_path(simulation_id: str, include_ext=True):
        ...
        zip_file_name = zip_file_name + (f".{constants.SIM_OUTPUT_FILE_EXTENSION}" if include_ext else "")
        return os.path.join(constants.SIMULATION_DATA_FOLDER, simulation_id, zip_file_name)
    ...
    def zip_output_files_to_simulation_folder_as_csvs(
        ...
        for fp in stage_output_file_paths:
            ...
            try:
                ...
                logger.exception("Exception occurred while writing csv's to output folder")
                raise e

        # get the zip file path
        zip_file_path = RADPFileSystemHelper.gen_sim_output_zip_file_path(simulation_id, include_ext=False)
        ...
        try:
            ...
            logger.exception(f"Exception occurred zipping files in simulation: {simulation_id}: {e}")
            raise e

    @staticmethod
    def get_stage_output_file_paths(stage: SimulationStage, hash_val: str, num_batches: int) -> List[str]:
        ...
        return [
            f"{file_path_without_batch}-{batch}.{constants.DF_FILE_EXTENSION}" for batch in range(1, num_batches + 1)
        ]

    @staticmethod
    def clear_output_data_from_stage(stage: SimulationStage, save_hash_val: Optional[str]):
        """
            ...
            if save_hash_val is None:
                ...
                for file_name in output_file_names:
                    if save_hash_val in file_name:
                        ...
                        os.remove(file_path)
                        delete_count += 1
            logger.info(f"Cleared {delete_count} unused outputs from stage: {stage.value}")
        except Exception as e:
    ...
    def load_model_metadata(model_id: str) -> Dict:
        ...
        try:
            ...
            logger.exception(f"Exception occurred loading metadata for model: {model_id}")
            raise e
