...
class RADPHelper:
    ...
    def resolve_model_status(
        ...
        while attempt < max_attempts:
            ...
            if not describe_model_response[constants.MODEL_EXISTS]:
                ...
                describe_model_response[constants.MODEL_STATUS]
                != constants.MODEL_TRAINED
            ):
    ...
    def resolve_simulation_status(
        ...
        attempt = 0
        while attempt < max_attempts:
            describe_simulation_response = self.radp_client.describe_simulation(
                simulation_id
            )
            if not describe_simulation_response[constants.SIMULATION_EXISTS]:
                print_if_verbose("Simulation not yet created", verbose)
            elif (
                describe_simulation_response[constants.SIMULATION_STATUS]
