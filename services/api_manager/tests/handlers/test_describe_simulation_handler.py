...
class TestDescribeSimulationHandler(TestCase):
    ...
    def test_handle_describe_simulation_request(self, mock_file_system_helper: MagicMock):
        mock_file_system_helper.load_simulation_metadata.return_value = {"metadata": "dummy"}
        assert DescribeSimulationHandler().handle_describe_simulation_request("dummy_simulation") == {
            "metadata": "dummy"
        }
        mock_file_system_helper.load_simulation_metadata.assert_called_once_with("dummy_simulation")

    @patch("api_manager.handlers.describe_simulation_handler.RADPFileSystemHelper")
    def test_handle_describe_simulation_request__simulation_not_found(self, mock_file_system_helper: MagicMock):
        mock_file_system_helper.side_effect
        mock_file_system_helper.load_simulation_metadata.side_effect = FileNotFoundError()
        with self.assertRaises(SimulationNotFoundException):
            DescribeSimulationHandler().handle_describe_simulation_request("dummy_simulation")
