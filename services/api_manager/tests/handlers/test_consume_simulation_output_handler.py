...
class TestConsumeSimulationOutputHandler(TestCase):
    ...
    @patch(
        "api_manager.handlers.consume_simulation_output_handler.RADPFileSystemHelper"
    )
    def test_handle_consume_simulation_output_request(
        self, mock_file_system_helper: MagicMock, mock_os: MagicMock
    ):
        mock_file_system_helper.gen_sim_output_zip_file_path.return_value = (
            "dummy_outzip_zip"
        )
        ...
        mock_file_system_helper.gen_sim_output_zip_file_path.assert_called_once_with(
            "dummy_sim"
        )
    ...
    def test_handle_consume_simulation_output_request__nonexistent_model(
        ...
        with self.assertRaises(SimulationOutputNotFoundException):
            assert ConsumeSimulationOutputHandler().handle_consume_simulation_output_request(
                "dummy_sim"
            )
